<x-guest-layout>
    ...
    <div class="max-w-fit mx-auto px-4 sm:px-6 lg:px-8 mt-14">
        <div class="pb-6 grid grid-cols-3 flex">
            @if ($tickets == true)
                    ...
                    <x-auth-card>
                        ...
                        <form method="POST" action="{{ route('update.ticket', [$ticket->id]) }}"
                            ...
                            <div class="mt-4 mb-6">
                                ...
                                @if ($ticket->file)
                                    <x-input type="text" class="block mt-1 w-full mb-4" name=""
                                        placeholder="Name" value="{{ ucwords($ticket->file->name) }}" readonly />
                                @else
                                    <x-input type="text" class="block mt-1 w-full mb-4" name=""
                                        placeholder="Name" value="No File Set" readonly />
                                @endif
        ...
        <div class="text-center">
            <form action="{{ route('update.linked', $tickets('id')) }}" method="POST">
                @csrf
                @method('PUT')
